...
parser =
  info (helper <*> argumentsP) $
    fullDesc <> progDesc "migrator." <> header "migrator - ."
...
globConfFileDef =
  GlobalOptions <$>
    strOption (
      long "config"
      <> short 'c'
      <> metavar "migratorCONF"
      <> value ""
      <> showDefault
      <> help "Global config file (default is ~/.migrator/config.yaml)."
    )
    <*>
    strOption (
      long "debug"
...
commandDefs =
  let
    cmdArray = [
      ...
      , ("delta", deltaOpts, "Calculate a delta of SQL state.")
      ]
...
deltaOpts :: Parser Command
deltaOpts =
  DeltaCmd <$> strArgument (metavar "INFILE" <> help "File containing the deltas.")
